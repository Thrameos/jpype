...
static PyObject *PyJPModule_convertBuffer(JPContext* context, JPPyBuffer& buffer, PyObject *dtype, PyObject *source);
...
static int PyJPModule_clear(PyObject *module)
{
	...
	Py_CLEAR(st->PyJPArrayIter_Type);
	...
	Py_CLEAR(st->class_magic_concrete);
...
static PyObject *PyJPModule_reflectMethod(PyObject* module, PyObject *obj)
{
	...
	if (cls == nullptr || cls != context->_java_lang_reflect_Method)
	{
		PyErr_Format(PyExc_TypeError,
	...
	JPMethodDispatch* dispatch = context->getTypeManager()->methodFromReflect(
			frame, PyJPValue_getJValue(frame, obj).l);
	if (dispatch == nullptr)
		return nullptr;
...
static PyObject *PyJPModule_arrayFromBuffer(PyObject *module, PyObject *args, PyObject *kwargs)
{
	...
	// we will do the same.
	{
		...
		if (buffer.valid())
			return PyJPModule_convertBuffer(context, buffer, dtype, source);
	}
	{
...
static int PyJPModule_traverse(PyObject *module, visitproc visit, void *arg)
{
	...
	Py_VISIT(st->PyJPArrayIter_Type);
	...
	Py_VISIT(st->class_magic_concrete);
...
static int PyJPModule_exec(PyObject* module)
{
	...
	JP_PY_CHECK();
	st->class_magic_concrete = PyDict_New();
	JP_PY_CHECK();
...
// PyJPModuleFault_check/PyJPModuleFault_throw are defined in pyjp_misc.cpp,
// alongside the _PyJPModule_fault_code state they operate on.
