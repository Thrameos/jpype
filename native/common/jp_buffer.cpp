...
{
	m_Object = frame.NewGlobalRef(value.getValue().l);
	m_Class = dynamic_cast<JPBufferType*>( value.getClass());
	JP_TRACE_IN("JPBuffer::JPBuffer");
	m_Address = frame.GetDirectBufferAddress(m_Object);
	m_Capacity = (Py_ssize_t) frame.GetDirectBufferCapacity(m_Object);
	m_Buffer.buf = m_Address;
	m_Buffer.format = m_Format;
	m_Format[0] = frame.orderBuffer(m_Object) ? '<' : '>';
	m_Format[1] = m_Class->getType()[0];
	m_Format[2] = 0;
	m_Buffer.itemsize = (Py_ssize_t) m_Class->getSize();
	m_Buffer.ndim = 1;
	m_Buffer.readonly = frame.isBufferReadOnly(m_Object);
