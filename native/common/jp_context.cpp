...
{
    // Default fallback to the core, guaranteed-to-exist primitive; refined
    // below to the active subinterpreter's own JVMNotRunning class when one
    // is available, since a bare process-wide _JVMNotRunning global would
    // hand out an exception class object belonging to a different
    // subinterpreter than the caller's.
    PyObject* jvm_not_running_exc = _JVMNotRunning;

    if (context != nullptr && context->modulestate != nullptr && context->modulestate->module_dict != nullptr)
    {
	...
	if (context == nullptr)
	{
		throw JPInternalError(jvm_not_running_exc, "Java Context is null", info);
	}

	if (!context->isRunning())
	{
		throw JPInternalError(jvm_not_running_exc, "Java Virtual Machine is not running", info);
	}
}
...
void JPContext::initializeResources(JNIEnv* env, bool interrupt)
{
	...
	// Back the JPJavaError/JPPythonError/JPInternalError hierarchy (see
	// jp_exception.h) -- ported here as static methods when the
	// JPypeContext singleton these used to be instance methods on was
	// retired in favor of NativeContext.
	m_Support_CreateExceptionID = frame.GetStaticMethodID(supportClass, "createException",
			"(JJ)Ljava/lang/Exception;");
	m_Support_GetExcClassID = frame.GetStaticMethodID(supportClass, "getExcClass",
			"(Ljava/lang/Throwable;)J");
	m_Support_GetExcValueID = frame.GetStaticMethodID(supportClass, "getExcValue",
			"(Ljava/lang/Throwable;)J");
	...
	// --- Bulk buffer/ragged array transfer (array-transfer-phase3) ---
