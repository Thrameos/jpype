...
JPProxy::~JPProxy()
{
	try
	{
		// m_ProxyType is a jref/GlobalPool handle (see JPProxy::JPProxy's
		// frame.storeGlobal above) - tryRelease no-ops safely if the owning
		// interpreter already tore down. m_Context is captured once at
		// construction, not resolved via JPContext_global, so this is safe
		// under multiple sub-interpreters.
