...
differences between JPype on Android and on a JVM.  The Android platform uses
...
of Python.  Thus, there is no need to launch the machine.  Instead to use jpype
only the statement ``import jpype`` is required.  In addition, the following
functions have been removed...

- ``jpype.startJVM`` is removed as the virtual machine cannot be started more
  than once.

- ``jpype.shutdownJVM`` is removed as DVM cannot be stopped during operation.
...
- ``jpype.beans`` has been removed as adding additional properties for semantic
  sugar increases the memory profile unnecessarily.

- attaching and detaching of threads is not allowed and those entry points have
  been removed.
...
- The dynamic class loader does not operate on Android as DVM does not support
  jar files.

- jpype.imports and jpype.JPackage use the jar file system to identify packages
  which is not available on DVM.  Therefore, all classes must be loaded using
  ``jpype.JClass``.
...
- Bytecode manipulation libraries do not function on DVM. This means generation
