...
5. Everything else (Object lifetime / Customization / Glossary) once the
   above exist. SPI and `python.io` have now landed, so the Customization
   chapter (including the new `$`-mangled direct dispatch sub-section) has
   real content to write against and no longer needs to wait.
