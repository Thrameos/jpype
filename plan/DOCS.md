...
| Known limitations | **Known limitations** — mirror the existing chapter's honesty; enumerate what's NOT implemented yet (subinterpreters, `python.io` — see `plan/IO.md`, any protocol gaps) rather than implying full parity with Python | not started, but cheap and high-value — do this early since the list already exists implicitly across memory files (see [[jpype_subinterpreter_difficulty]] for the subinterpreter gap specifically) |
