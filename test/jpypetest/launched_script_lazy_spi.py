"""
End-to-end smoke test for the SPI's lazy per-module resolution, run from
a plain launched Python script (jpype.startJVM()) rather than the
Java-hosted NGTest harness used by `mvn test` (that direction is covered
by PyLazySpiNGTest).
...
the "no explicit invocation" property: adding a class under
WrapperService#getResources() with lazy: true is the only thing a
...
pattern) -- run directly, e.g. `python launched_script_lazy_spi.py`.
"""
import io
